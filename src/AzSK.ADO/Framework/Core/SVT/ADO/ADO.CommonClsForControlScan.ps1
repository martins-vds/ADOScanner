...
class CommonClsForControlScan: ADOSVTBase {
    ...
    hidden [ControlResult] CheckFeedAccess([ControlResult] $controlResult)
    {
        try
        {
            ...
            $feedsPermission = ($feedPermissionObj | Where-Object {$_.role -eq "administrator" -or $_.role -eq "contributor"}) | Select-Object -Property @{Name="FeedName"; Expression = {$this.ResourceContext.ResourceName}},@{Name="Role"; Expression = {$_.role}},@{Name="DisplayName"; Expression = {$_.displayName}} ;
            $feedsWithUploadPkgPermission = $feedsPermission | Where-Object { $GroupsToCheckForFeedPermission -contains $_.DisplayName.split('\')[-1] }
              
            if ($feedsWithUploadPkgPermission) 
            {
                $controlResult.AddMessage("Note: The following groups are considered as 'critical': [$($GroupsToCheckForFeedPermission -join ", ")]");
                $controlResult.AddMessage([VerificationResult]::Failed, "Feed have contributor/administrator permission: ");
    ...
    hidden [ControlResult] TestMethod([ControlResult] $controlResult) {
        $controlResult.AddMessage([VerificationResult]::Failed, "Trusted repository.");
        return $controlResult;
    }

    hidden [ControlResult] TestMethodArtifact([ControlResult] $controlResult) {
        $controlResult.AddMessage([VerificationResult]::Passed, "Trusted artifact.");
        return $controlResult;
    }
}
